...
Sophus::SE3f Tracking::GrabImageDeepStereo(const cv::Mat& imRectLeft, const cv::Mat& imRectRight, const cv::Mat& imRectConf, const double& timestamp, string filename)
{
    // cout << "GrabImageStereo" << endl;
    ...
    if (mSensor == System::STEREO && !mpCamera2)
        ...
        mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, mpCamera2, mTlr);
    else if (mSensor == System::IMU_STEREO && !mpCamera2 && !imRectConf.empty())
        mCurrentFrame = Frame(mImGray, imGrayRight, imRectConf, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, &mLastFrame, *mpImuCalib);
    else if (mSensor == System::IMU_STEREO && !mpCamera2)
        mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, &mLastFrame, *mpImuCalib);
    else if (mSensor == System::IMU_STEREO && mpCamera2 && !imRectConf.empty())
        mCurrentFrame = Frame(mImGray, imGrayRight, imRectConf, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, mpCamera2, mTlr, &mLastFrame, *mpImuCalib);
    else if (mSensor == System::IMU_STEREO && mpCamera2)
        mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpCamera, mpCamera2, mTlr, &mLastFrame, *mpImuCalib);
        ...
        if (!mbOnlyTracking) {
            ...
            if (mState == OK) {
                ...
                if ((!mbVelocity && !pCurrentMap->isImuInitialized()) || mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
                    Verbose::PrintMess("TRACK: Track with respect to the reference KF ", Verbose::VERBOSITY_DEBUG);
                    // bOK = TrackReferenceKeyFrame();
                    bOK = TrackReferenceDeepKeyFrame(); // Deep
                } else {
                    Verbose::PrintMess("TRACK: Track with motion model", Verbose::VERBOSITY_DEBUG);
                    // 用恒速模型跟踪。所谓的恒速就是假设上上帧到上一帧的位姿=上一帧的位姿到当前帧位姿
                    // 根据恒速模型设定当前帧的初始位姿，用最近的普通帧来跟踪当前的普通帧
                    // 通过投影的方式在参考帧中找当前帧特征点的匹配点，优化每个特征点所对应3D点的投影误差即可得到位姿
                    // bOK = TrackWithMotionModel();
                    bOK = TrackDeepWithMotionModel(); // Deep
                    if (!bOK)
                        // bOK = TrackReferenceKeyFrame(); // 根据恒速模型失败了，只能根据参考关键帧来跟踪
                        bOK = TrackReferenceDeepKeyFrame(); // 根据恒速模型失败了，只能根据参考关键帧来跟踪Deep
                }
                // 新增了一个状态RECENTLY_LOST，主要是结合IMU看看能不能拽回来
                // Step 6.3 如果经过跟踪参考关键帧、恒速模型跟踪都失败的话，并满足一定条件就要标记为RECENTLY_LOST或LOST
                if (!bOK) {
        ...
        } else // 纯定位模式
        {
            ...
            if (mState == LOST) {
                ...
                if (!mbVO) {
                    ...
                    if (mbVelocity) {
                        // bOK = TrackWithMotionModel();
                        bOK = TrackDeepWithMotionModel();
                    } else {
                        bOK = TrackReferenceDeepKeyFrame();
                    }
                } else {
                    // In last frame we tracked mainly "visual odometry" points.
                    ...
                    if (mbVelocity) {
                        // bOKMM = TrackWithMotionModel();
                        bOKMM = TrackDeepWithMotionModel();
                        vpMPsMM = mCurrentFrame.mvpMapPoints;
                        vbOutMM = mCurrentFrame.mvbOutlier;
                        TcwMM = mCurrentFrame.GetPose();
                    }
                    // bOKReloc = Relocalization();
                    bOKReloc = RelocalizationDeep();
 ...
 *
 */
bool Tracking::TrackReferenceDeepKeyFrame()
{
    // Compute Bag of Words vector
    // Step 1：将当前帧的描述子转化为BoW向量
    mCurrentFrame.ComputeBoW();
    ...
    // cout << " TrackReferenceKeyFrame mLastFrame.mTcw:  " << mLastFrame.mTcw << endl;
    // Optimizer::PoseOptimization(&mCurrentFrame);
    Optimizer::PoseOptimizationDeep(&mCurrentFrame);
...
bool Tracking::TrackDeepWithMotionModel()
{
    ORBmatcher matcher(0.9, true);
 ...
 * Step 5：决定是否跟踪成功
 */
bool Tracking::TrackDeepLocalMap()
{
    ...
    if (!mpAtlas->isImuInitialized())
        ...
        } else // 如果积累的IMU数据量比较多，考虑使用IMU数据优化
        {
            ...
            {
                Verbose::PrintMess("TLM: PoseInertialOptimizationLastFrame ", Verbose::VERBOSITY_DEBUG);
                inliers = Optimizer::PoseInertialOptimizationDeepLastFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetIniertialBA1());
            } else {
                Verbose::PrintMess("TLM: PoseInertialOptimizationLastKeyFrame ", Verbose::VERBOSITY_DEBUG);
                inliers = Optimizer::PoseInertialOptimizationDeepLastKeyFrame(&mCurrentFrame); // , !mpLastKeyFrame->GetMap()->GetIniertialBA1());
            }
        }
    }
...
bool Tracking::RelocalizationDeep()
{
    Verbose::PrintMess("Starting relocalization", Verbose::VERBOSITY_NORMAL);
    // Compute Bag of Words Vector
    mCurrentFrame.ComputeBoW();
    ...
    while (nCandidates > 0 && !bMatch) {
        for (int i = 0; i < nKFs; i++) {
            ...
            if (bTcw) {
                ...
                // int nGood = Optimizer::PoseOptimization(&mCurrentFrame);
                int nGood = Optimizer::PoseOptimizationDeep(&mCurrentFrame);
                ...
                if (nGood < 50) {
                    ...
                    if (nadditional + nGood >= 50) {
                        // nGood = Optimizer::PoseOptimization(&mCurrentFrame);
                        nGood = Optimizer::PoseOptimizationDeep(&mCurrentFrame);
