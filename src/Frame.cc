...
Frame::Frame(const cv::Mat& imLeft, const cv::Mat& imRight, const cv::Mat& imConf, const double& timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, GeometricCamera* pCamera, Frame* pPrevF, const IMU::Calib& ImuCalib)
    : mpcpi(NULL)
    , mpORBvocabulary(voc)
    , mpORBextractorLeft(extractorLeft)
    , mpORBextractorRight(extractorRight)
...
void Frame::SaveConfidence(const cv::Mat& imConf)
{
    mvConf = vector<float>(mvKeys.size(), 1.0f);
    if (!imConf.empty()) {
        for (int i = 0; i < mvKeys.size(); i++) {
            mvConf[i] = imConf.at<float>(mvKeys[i].pt);
        }
    }
}
...
Frame::Frame(const cv::Mat& imLeft, const cv::Mat& imRight, const cv::Mat& imConf, const double& timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, GeometricCamera* pCamera, GeometricCamera* pCamera2, Sophus::SE3f& Tlr, Frame* pPrevF, const IMU::Calib& ImuCalib)
    : mpcpi(NULL)
    , mpORBvocabulary(voc)
    , mpORBextractorLeft(extractorLeft)
    , mpORBextractorRight(extractorRight)
