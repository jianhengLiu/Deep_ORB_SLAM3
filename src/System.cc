    ...
    , mbShutDown(false)
{
    ...
    // 设置最远3D地图点的深度值，如果超过阈值(20m)，说明可能三角化不太准确，丢弃
    if (settings_)
...
Sophus::SE3f System::TrackDeepStereo(const cv::Mat& imLeft, const cv::Mat& imRight, const cv::Mat& imConf, const double& timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    if (mSensor != STEREO && mSensor != IMU_STEREO) {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to Stereo nor Stereo-Inertial." << endl;
        exit(-1);
    ...
    cv::Mat imLeftToFeed, imRightToFeed, imConfTofeed;
    if (settings_ && settings_->needToRectify()) {
        cv::Mat M1l = settings_->M1l();
        cv::Mat M2l = settings_->M2l();
        cv::Mat M1r = settings_->M1r();
        ...
        cv::remap(imLeft, imLeftToFeed, M1l, M2l, cv::INTER_LINEAR);
        cv::remap(imRight, imRightToFeed, M1r, M2r, cv::INTER_LINEAR);
        cv::remap(imConf, imConfTofeed, M1r, M2r, cv::INTER_LINEAR);
    } else if (settings_ && settings_->needToResize()) {
        cv::resize(imLeft, imLeftToFeed, settings_->newImSize());
        cv::resize(imRight, imRightToFeed, settings_->newImSize());
        cv::resize(imConf, imConfTofeed, settings_->newImSize());
    } else {
        imLeftToFeed = imLeft.clone();
        imRightToFeed = imRight.clone();
        imConfTofeed = imConf.clone();
    }
    ...
    // std::cout << "start GrabImageStereo" << std::endl;
    Sophus::SE3f Tcw = mpTracker->GrabImageDeepStereo(imLeftToFeed, imRightToFeed, imConfTofeed, timestamp, filename);
