...
public:
    ...
    // Constructor for deep stereo cameras.
    Frame(const cv::Mat& imLeft, const cv::Mat& imRight, const cv::Mat& imConf, const double& timeStamp, ORBextractor* extractorLeft, ORBextractor* extractorRight, ORBVocabulary* voc, cv::Mat& K, cv::Mat& distCoef, const float& bf, const float& thDepth, GeometricCamera* pCamera, Frame* pPrevF = static_cast<Frame*>(NULL), const IMU::Calib& ImuCalib = IMU::Calib());
...
public:
    ...
    std::vector<float> mvConf;

    // Corresponding stereo coordinate and depth for each keypoint.
    std::vector<MapPoint*> mvpMapPoints;
    // "Monocular" keypoints have a negative value.
    std::vector<float> mvuRight; //特征点对应在右目中的坐标(u)
...
private:
    ...
    void SaveConfidence(const cv::Mat& imConf);
...
public:
    // mpCamera2用于KannalaBrandt8相机模型
    // http://zhaoxuhui.top/blog/2021/08/01/orb-slam3-note-4-camera-model-and-feature-extraction.html
    ...
    cv::Mat imgLeft, imgRight, imgConf;
