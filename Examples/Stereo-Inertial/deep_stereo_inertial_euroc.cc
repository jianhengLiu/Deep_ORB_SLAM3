...
void LoadImages(const string& strPathLeft, const string& strPathRight, const string& strPathConf, const string& strPathTimes,
    vector<string>& vstrImageLeft, vector<string>& vstrImageRight, vector<string>& vstrConf, vector<double>& vTimeStamps);
...
int main(int argc, char** argv)
{
    if (argc < 5) {
        cerr << endl
             << "Usage: ./deep_stereo_inertial_euroc path_to_vocabulary path_to_settings path_to_sequence_folder_1 path_to_times_file_1 (path_to_image_folder_2 path_to_times_file_2 ... path_to_image_folder_N path_to_times_file_N) " << endl;
        return 1;
    }
    ...
    for (seq = 0; seq < num_seq; seq++) {
        ...
        string pathConf = pathSeq + "/mav0/cam0/conf_fine";
        ...
        cout << "LOADED IMU: " << vTimestampsImu[seq].size() << "data!" << endl;
    ...
    for (seq = 0; seq < num_seq; seq++) {
        ...
        for (int ni = 0; ni < nImages[seq]; ni++, proccIm++) {
            ...
            imConf = cv::imread(vstrConf[seq][ni], cv::IMREAD_UNCHANGED);
            ...
            if (ni > 0)
                ...
                {
                    vImuMeas.emplace_back(vAcc[seq][first_imu[seq]].x, vAcc[seq][first_imu[seq]].y, vAcc[seq][first_imu[seq]].z,
    ...
    vector<string>& vstrImageLeft, vector<string>& vstrImageRight, vector<string>& vstrConf, vector<double>& vTimeStamps)
{
    ...
    while (!fTimes.eof()) {
        ...
        if (!s.empty()) {
            ...
            vstrConf.push_back(strPathConf + "/" + ss.str() + ".tiff");
            double t;
            ss >> t;
...
void LoadIMU(const string& strImuPath, vector<double>& vTimeStamps, vector<cv::Point3f>& vAcc, vector<cv::Point3f>& vGyro)
{
    ...
    while (!fImu.eof()) {
        ...
        if (!s.empty()) {
            ...
            vAcc.emplace_back(data[4], data[5], data[6]);
            vGyro.emplace_back(data[1], data[2], data[3]);
        }
    }
}
