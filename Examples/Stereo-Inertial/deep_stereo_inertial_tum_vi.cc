 ...
 * @LastEditTime: 2022-04-15 16:06:26
...
int main(int argc, char** argv)
{
    ...
    vector<vector<string>> vstrImageConfFilenames;
    ...
    vstrImageConfFilenames.resize(num_seq);
    ...
    for (seq = 0; seq < num_seq; seq++) {
        ...
        for (int ni = 0; ni < nImages[seq]; ni++, proccIm++) {
            ...
            if (imageScale != 1.f) {
                ...
                cv::resize(imConf, imConf, cv::Size(width, height));
#ifdef REGISTER_TIMES
#ifdef COMPILEDWITHC11
    ...
    vector<string>& vstrImageLeft, vector<string>& vstrImageRight, vector<string>& vstrImageConf, vector<double>& vTimeStamps)
{
    ...
    cout << strPathConf << endl;
    cout << strPathTimes << endl;
    fTimes.open(strPathTimes.c_str());
    vTimeStamps.reserve(5000);
    vstrImageLeft.reserve(5000);
    vstrImageRight.reserve(5000);
    vstrImageConf.reserve(5000);
    while (!fTimes.eof()) {
        string s;
        getline(fTimes, s);
